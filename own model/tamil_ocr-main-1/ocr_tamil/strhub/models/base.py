...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def validation_step(self, batch, batch_idx) -> Optional[STEP_OUTPUT]:
        ...
        # Store outputs for aggregation in on_validation_epoch_end
    ...
    def on_validation_epoch_end(self) -> None:
        """Called at the end of validation epoch (PyTorch Lightning v2.0+)."""
        # Get outputs from validation_step - stored by Lightning
        outputs = self.trainer.callback_metrics
        
        # Calculate metrics from stored outputs if available
        if hasattr(self, '_validation_outputs'):
            acc, ned, loss = self._aggregate_results(self._validation_outputs)
    ...
    def configure_optimizers(self):
        ...
        # Create OneCycleLR scheduler with warmup
