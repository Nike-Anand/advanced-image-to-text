...
def download(url: str, dest_folder: str):
    ...
    if not os.path.exists(file_path):
        try:
            ...
            print(r"You can also manually download the file from github and keep under home_folder\.model_weights")
            os.remove(file_path)
    ...
    def load_model(self):
        ...
        if self.fp16:
            try:
                ...
                print("Warning: English model not available, using Tamil model only")
                self.eng_parseq = None
    ...
    def text_recognize_batch(self,exported_regions):
        ...
        for data in dataloader:
            ...
            if "tamil" in self.lang:
                ...
                tamil_label_list.extend(["" for i in range(len(data))])
                tamil_confidence_list.extend([torch.tensor(-1.0) for i in range(len(data))])


            # english prediction
            # eng_preds, eng_confidence = self.read_english_batch(data)
            if "english" in self.lang and self.eng_parseq is not None:
                with torch.cuda.amp.autocast() and torch.inference_mode():
                ...
                eng_label_list.extend(["" for i in range(len(data))])
                eng_confidence_list.extend([torch.tensor(-1.0) for i in range(len(data))])
